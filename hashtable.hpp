...
string get(hashTable& ht, const string& key, int index) { // получение значения элемента по ключу
    unsigned hashIndex = func(key); // индекс через хэш-функцию

    if (hashIndex >= 100) { // Проверка на корректность hashIndex
        cout << "Некорректный индекс.\n";
        return "";
    }

    hNode* current = ht.table[hashIndex];   // указатель на первый элемент цепочки
    int count = 0; // позиция узла
    ...
    while (current) { // ищем узел
        if (current->key == key) {
            ...
            count++; // увеличиваем счетчик при нахождении совпадения ключа
        }
...
void del(hashTable& ht, const string& key) { // удаление элемента по ключу
    ...
    while (current && current->key != key) { // поиск нужного узла
        newNode = current;         // Зновый указатель указывает на предыдущий узел
        current = current->next;
    }
